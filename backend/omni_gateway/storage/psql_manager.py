...
class PSQLManager:
    ...
    async def initialize(self) -> None:
        ...
        async with self._lock:
            ...
            try:
                self._dsn = os.getenv("OGW_POSTGRESQL_URI", "")
                if not self._dsn:
                    raise RuntimeError("OGW_POSTGRESQL_URI environment variable is not set")
    ...
    async def _create_tables(self, conn: asyncpg.Connection) -> None:
        ...
        """)
        await conn.execute("""
            CREATE INDEX IF NOT EXISTS idx_ag_disabled ON omni_credentials(disabled)
        """)
        await conn.execute("""
            CREATE INDEX IF NOT EXISTS idx_ag_rotation_order ON omni_credentials(rotation_order)
        """)
