...
Specialized owners (OIDC, HA, telemetry) still enforce cross-field security rules. This module
owns field names, scalar parsing, defaults, mutability, grouping, and safe public metadata.
"""
...
WORKERS=1
OMNI_EXPERIMENTAL_COORDINATION=false
...
_BOOLEAN_NAMES = frozenset(
    """OMNI_EXPERIMENTAL_COORDINATION PANEL_COOKIE_SECURE TRUST_PROXY_HEADERS OIDC_ENABLED
    ...
    TOKEN_COMPRESSION_ENABLED PROMETHEUS_EXPORT_ENABLED OTEL_EXPORT_ENABLED""".split()
)
_INTEGER_NAMES = frozenset(
    """PORT HOST_PORT OMNI_REPLICA_COUNT WORKERS OMNI_COORDINATION_EPOCH
...
_SECRET_NAMES = frozenset(
    ...
    OMNI_COORDINATION_KEY MONGODB_URI POSTGRESQL_URI REDIS_URL CODE_ASSIST_CREDENTIALS_JSON
...
_EXPERIMENTAL_NAMES = frozenset(
    """OMNI_RUNTIME_MODE OMNI_REPLICA_COUNT WORKERS OMNI_EXPERIMENTAL_COORDINATION
    OMNI_COORDINATION_NAMESPACE OMNI_DEPLOYMENT_ID OMNI_COORDINATION_KEY
    OMNI_COORDINATION_EPOCH REDIS_URL""".split()
)
_BASIC_NAMES = frozenset(
    """HOST PORT HOST_PORT CORS_ORIGINS CORS_ORIGIN_REGEX API_KEY PANEL_PASSWORD SETUP_TOKEN
...
_LIMITS: dict[str, tuple[int | float | None, int | float | None]] = {
    ...
    "OMNI_COORDINATION_EPOCH": (1, 1_000_000_000),
...
_CHOICES = {
    "OMNI_RUNTIME_MODE": ("standalone", "coordinated"),
    "ROUTING_STRATEGY": (
        "balanced",
        "priority",
        "weighted",
