...
from log import log
from core.converter.openai2gemini import _convert_usage_metadata
...
def extract_fake_stream_content(response: Any) -> Tuple[str, str, Dict[str, int]]:
    ...
    try:
        ...
        if not content and reasoning_content:
            ...
            content = "[The model returned only reasoning content. Please retry or rephrase the request.]"
...
def build_gemini_fake_stream_chunks(content: str, reasoning_content: str, finish_reason: str, images: List[Dict[str, Any]] = None, chunk_size: int = 50) -> List[Dict[str, Any]]:
    ...
    if not content:
        default_text = "[The model returned only reasoning content. Please retry or rephrase the request.]" if reasoning_content else "[The upstream response was empty. Please retry.]"
