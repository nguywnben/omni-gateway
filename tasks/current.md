...
- Implementation baseline: W4.16 Task 5 (`feat(identity): move sessions behind coordination`).
...
- Control state: **IN PROGRESS — W4.16 TASK 6 NEXT**.
- Execution mode: continuous through the remaining W4.16–W4.19 queue under
  ...
  boundaries.
- Expected worktree state at this checkpoint: clean after the W4.16 Task 5 implementation commit.
...
- Last verified full suite: 1,180 tests passed on Python 3.14.6 with 25 opt-in live backend tests
  skipped because no test URI was configured. The W4.16 Task 5 focused matrix passes 96 tests;
...
Execute W4.16 Task 6 from
`docs/superpowers/plans/2026-09-02-w4.16-identity-security-coordination.md`: replace process-local
login, recovery, and OIDC-start throttles with atomic coordination while standalone remains
in-memory.
