...
При первом запуске экран настройки проверяет хранилище, адрес консоли, транспорт/cookie и состояние установки, прежде чем разрешить создание владельца. Пароля по умолчанию нет. Для прямой настройки через localhost токен не нужен. Перед удалённой настройкой задайте уникальный `SETUP_TOKEN` длиной не менее 24 символов и перезапустите сервис; приложение не создаёт и не записывает этот секрет в журнал. Пароль владельца должен содержать 12–256 символов.
...
| `SETUP_TOKEN` | пусто | Обязателен перед первой удалённой настройкой; используйте уникальное значение длиной не менее 24 символов. Приложение его не создаёт и не журналирует. Для прямого localhost не нужен. |
