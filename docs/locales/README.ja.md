...
初回起動時、セットアップ画面は所有者の作成を有効にする前に、ストレージ、コンソールアドレス、通信／Cookie、インストール状態を確認します。既定のパスワードはありません。localhost から直接設定する場合、トークンは不要です。リモート設定の前に、24 文字以上の固有な `SETUP_TOKEN` を設定してサービスを再起動してください。アプリケーションはこの秘密を生成もログ出力もしません。所有者パスワードは 12～256 文字にしてください。
...
| `SETUP_TOKEN` | 空 | リモート初回セットアップ前に必須。24 文字以上の固有値を使用してください。アプリケーションは生成もログ出力もしません。localhost 直接設定では不要です。 |
