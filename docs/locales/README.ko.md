...
최초 실행 시 설정 화면은 소유자 생성을 활성화하기 전에 저장소, 콘솔 주소, 전송/쿠키 및 설치 상태를 확인합니다. 기본 비밀번호는 없습니다. localhost 직접 설정에는 토큰이 필요하지 않습니다. 원격 설정 전에는 24자 이상의 고유한 `SETUP_TOKEN`을 지정하고 서비스를 다시 시작하세요. 애플리케이션은 이 비밀을 생성하거나 로그에 기록하지 않습니다. 소유자 비밀번호는 12–256자여야 합니다.
...
| `SETUP_TOKEN` | 비어 있음 | 원격 최초 설정 전에 필수입니다. 24자 이상의 고유값을 사용하세요. 애플리케이션은 이를 생성하거나 기록하지 않습니다. localhost 직접 설정에는 필요 없습니다. |
