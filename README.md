...
> **Project status:** Stable. Version `1.2.0` adds Codex and OpenAI Platform integrations, provider-native credential diagnostics, and a scalable provider catalog while preserving the stable SDK routes, canonical management routes, configuration names, and single-instance runtime contract established in `1.0.0`.
...
sudo docker run -d \
  ...
  nguywnben/omni-gateway:1.2.0
```

The same release is published to GitHub Packages as `ghcr.io/nguywnben/omni-gateway:1.2.0`. The `latest` tag tracks the newest stable release; `edge` tracks verified but unreleased builds from `main`. Pin a version tag or digest when reproducible deployment matters.
...
The `1.2.0` container is published for `linux/amd64`. ARM64 publication is intentionally paused until every provider dependency, including the Vertex transport stack, can be built and tested with the same contract.
...
The included compose file pulls `nguywnben/omni-gateway:latest` and uses `/opt/omni-gateway` by default for persistent host data. Set `IMAGE=nguywnben/omni-gateway:1.2.0` to pin this release, and set `DATA_DIR=/custom/path` when the server uses a different storage location.
